...
module State = struct
  ...
  let pp ppf (state : t) =
    ...
    Fmt.pf ppf "@[<hov>stack:[%a];@;ρ:%a@]@." Stack.pp stack pp_map locals
end
...
let join lhs rhs = match lhs with Some x -> Some x | None -> rhs
...
module Control_flow (DS : DATA_STATE) = struct
  ...
  and eval_instr :
   ...
   fun ~no_input state instr ->
    ...
    | Block (_str_opt, _bt, body) ->
      ...
      let new_state = join (JumpTarget.find_opt (I 0) mapping) res in
      (new_state, m_decr)
    ...
    | Br_if id -> (
      let v, stack = Stack.pop state.stack in
      match v with
      | I32 0l -> (Some { state with stack }, JumpTarget.empty)
      | _ ->
        (* br i *)
    ...
    | Br_table (cases, default) ->
      ...
      (None, JumpTarget.of_list [ (I jt, { state with stack }) ])
...
module Data_state : DATA_STATE = struct
  ...
  let eval_i32 (state : State.t) : Binary.i32_instr -> _ = function
    | Binary.Const i ->
      let stack = Stack.push_i32 state.stack i in
      { state with stack }
    | Add ->
      let stack = Stack.apply_i32_i32_i32 state.stack Value.I32.add in
      { state with stack }
    | Sub ->
      let stack = Stack.apply_i32_i32_i32 state.stack Value.I32.sub in
      { state with stack }
    | _ -> assert false

  let eval_i64 (state : State.t) : Binary.i64_instr -> _ = function
    | Binary.Const i ->
      let stack = Stack.push_i64 state.stack i in
      { state with stack }
    | Add ->
      let stack = Stack.apply_i64_i64_i64 state.stack Value.I64.add in
      { state with stack }
    | Sub ->
      let stack = Stack.apply_i64_i64_i64 state.stack Value.I64.sub in
      { state with stack }
    | _ -> assert false

  let eval_local (state : State.t) : Binary.local_instr -> _ = function
    | Get i -> (
      match Locals.find_opt i state.locals with
      | None -> Fmt.failwith "local.get: local %i is not set" i
      | Some v ->
        let stack = v :: state.stack in
        { state with stack } )
    | Set i ->
      let v, stack = Stack.pop state.stack in
      let locals = Locals.add i v state.locals in
      { stack; locals }
    | Tee i ->
      let v, stack = Stack.pop state.stack in
      let locals = Locals.add i v state.locals in
...
module Interpreter = Control_flow (Data_state)
...
let run ~no_input (m : Binary.Module.t) =
  ...
  let state : State.t = { stack = Stack.empty; locals = Locals.empty } in
  let res, _ = Interpreter.eval_expr ~no_input state start.body.raw in
